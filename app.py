...
def render_whatsapp_feature(encoded_msg, key_prefix):
  ...
  if wa_lock_setting == "Free" or is_prem:
    ...
    # युनिक आयडी तयार केला जेणेकरून duplicate key चा एरर कधीच येणार नाही
...
# --------------------------------------------------
    ...
    # --------------------------------------------------
      ...
      total_planned_days = (
          sum(t["planned_duration"] for t in tasks) if tasks else 0
      )
      total_critical_delay = (
          sum(t["delay_days"] for t in tasks if t["is_critical"] == 1)
          if tasks
          else 0
      )
      ...
      # ५. व्हॉट्सॲप मेसेज तयार करणे व शेअर करणे (Timeline ब्लॉकच्या आत)
      st.write("---")
      wa_timeline_text = (
