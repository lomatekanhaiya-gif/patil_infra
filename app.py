...
def init_db():
    ...
    # 10. Pre-Concreting Checklist Table (NEW FEATURE)
    cursor.execute("""
...
if st.session_state.selected_module is None:
    ...
    with main_col2:
        ...
        button_text = (
            "🔼 Close Estimator Tools"
            if st.session_state.show_estimator_menu
            else "📐 Open Estimator Tools"
        )
        if st.button(
            button_text, key="btn_toggle_est_menu", use_container_width=True
        ):
            st.session_state.show_estimator_menu = (
                not st.session_state.show_estimator_menu
            )
            st.rerun()
    ...
    if st.session_state.show_estimator_menu:
        st.write("---")
        st.markdown(
            "##### 🔽 खालीलपैकी एक Estimator टूल निवडा:",
            unsafe_allow_html=True,
        )
        ...
        with col_icon1:
            ...
            if st.button(
                "🧮 Calculator", key="btn_open_calc", use_container_width=True
            ):
        ...
        with col_icon3:
            ...
            if st.button(
                "🏗️ Open BBS", key="btn_open_bbs", use_container_width=True
            ):
...
# ==========================================
    ...
    if st.button(
        "⬅️ मुख्य मेनूवर जा (Back to Main)", key="btn_back_to_main_calc"
    ):
        st.session_state.selected_module = None
        st.rerun()
...
# ==========================================
    ...
    if st.button(
        "⬅️ मुख्य मेनूवर जा (Back to Main)", key="btn_back_to_main_bbs"
    ):
        st.session_state.selected_module = None
        st.rerun()
...
# ==========================================
    ...
    if st.button(
        "⬅️ मुख्य मेनूवर जा (Back to Main)", key="btn_back_to_main_qs"
    ):
        st.session_state.selected_module = None
        st.rerun()
...
# ==========================================
    ...
    # 5th Tab Added for Pre-Concreting Checklist
    site_tab1, site_tab2, site_tab3, site_tab4, site_tab5 = st.tabs([
        "👷 1. Attendance & Wages",
        "📦 2. Material Inventory",
        "📸 3. Progress Report",
        "📊 4. Weekly Dashboard",
        "🏗️ 5. Pre-Concreting Checklist",
    ])
    ...
    # --------------------------------------------------
    # TAB 5: PRE-CONCRETING CHECKLIST (NEWLY ADDED FEATURE)
    # --------------------------------------------------
    with site_tab5:
        st.markdown(
            "#### 🏗️ Pre-Concreting Checklist (स्लॅब भरण्यापूर्वीची डिजिटल"
            " चेकलिस्ट)"
        )
        st.caption(
            "💡 काँक्रीटिंग किंवा स्लॅब भरण्यापूर्वी साईट इंजिनिअरने खालील सर्व"
            " बाबी तपासून टिक-मार्क करणे आवश्यक आहे."
        )

        # डीफॉल्ट चेकलिस्ट आयटम्स
        default_chk_items = [
        ...
        # डेटाबेस मधून सध्याचे आयटम्स घेणे
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
        ...
        # जर डेटाबेसमध्ये अजून एकही आयटम नसेल, तर डीफॉल्ट आयटम इन्सर्ट करा
        if not db_items:
        ...
        # प्रोग्रेस कार्ड आणि स्टेटस वॉर्निंग
        st.markdown(
            f"""
        ...
        # नवीन तपासणी पॉईंट जोडण्यासाठी इनपुट बॉक्स
