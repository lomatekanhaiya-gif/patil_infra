...
def auto_cleanup_30day_attendance():
    """३० दिवसांपेक्षा जुना हजेरीचा डेटा आपोआप डिलीट करणारा ऑटोमॅटिक क्लीनअप इंजिन"""
    try:
        ...
        cursor.execute(
            "DELETE FROM labor_attendance_records WHERE date < ?",
            (cutoff_date,),
        )
        conn.commit()
        conn.close()
...
def init_db():
    ...
    # 10. Pre-Concreting Checklist Table
    cursor.execute("""
    ...
    # 11. Labor Master Table (न्यू फीचर)
    cursor.execute("""
    ...
    # 12. Site Geo-Fence Config Table (न्यू फीचर)
    cursor.execute("""
    ...
    # 13. Dynamic Attendance Tokens Table (न्यू फीचर)
    cursor.execute("""
    ...
    # 14. Labor Attendance Live Verification Records Table (न्यू फीचर - Tamper-Proof & 30-Day Auto Delete Engine)
    cursor.execute("""
...
# Dynamic Link labor direct entry interceptor
if "att_token" in query_params:
    st.session_state["public_att_token"] = query_params["att_token"]
...
if st.session_state.app_user_name is None and "saved_user" in query_params:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT user_key FROM users WHERE user_key = ?", (saved_key,)
    )
    row = cursor.fetchone()
    conn.close()
    if row:
...
if is_curr_premium:
    ...
    bg_gradient = (
        "linear-gradient(135deg, #030712 0%, #0b0f19 50%, #020617 100%)"
    )
                ...
                if st.button(
                    ...
                    if row:
                        ...
                        if c_info.get("used") == 1:
                            st.error(
                                "❌ हा कोड आधीच वापरला गेला आहे! तो आता व्हॅलिड"
                                " नाही."
                            )
                            conn.close()
                        else:
                            exp_datetime = get_ist_time() + datetime.timedelta(
                                days=28
                            )
                            exp_str = exp_datetime.strftime("%Y-%m-%d %H:%M:%S")
                            now_str = get_ist_time().strftime(
                                "%Y-%m-%d %H:%M:%S"
                            )

                            cursor.execute(
                                "UPDATE premium_codes SET used = 1, used_by ="
                                " ?, used_date = ? WHERE code = ?",
                                (current_user_name, now_str, p_code),
                            )

                            disp_name = (
                                current_user_name if current_user_name else ""
                            )
                            welcome_msg = (
                                f"{disp_name} मी कन्हैया आपले पाटील इन्फ्राटेक मध्ये"
                        ...
                        conn.close()
                        st.error(
                            "❌ चुकीचा प्रिमियम कोड! कृपया अचूक कोड टाका."
                        )
...
# ==========================================
# 🌐 PUBLIC LABOR DYNAMIC LINK VERIFICATION FLOW (INTERCEPTOR)
# ==========================================
if "public_att_token" in st.session_state:
    p_token = st.session_state["public_att_token"]
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM attendance_tokens WHERE token = ?", (p_token,)
    )
    ...
    st.markdown(
        ...
        <div class="main-header">
            <h1 style='color: white; margin:0; font-size: 26px; font-weight: 800;'>👷‍♂️ LABOR GEO-ATTENDANCE PORTAL</h1>
            <p style='color: #e0f2fe; margin:5px 0 0 0; font-size: 14px;'>PATIL INFRATECH LIVE VERIFICATION</p>
        </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    if not tok_row:
        st.error(
            "⚠️ हा हजेरीचा टोकन किंवा लिंक अवैध आहे / एक्सपायर झाली आहे!"
        )
        st.stop()

    tok_info = dict(tok_row)
    eng_key = tok_info["user_key"]
    att_type = tok_info["type"]  # IN or OUT

    # Get Geo Config
    cursor.execute(
        "SELECT * FROM site_geo_config WHERE user_key = ?", (eng_key,)
    )
    ...
    if not geo_row:
        st.error(
            "⚠️ साईट इंजिनिअरने अजून लाईव्ह जीपीएस लोकेशन सेट केले नाही!"
        )
        st.stop()
    ...
    st.info(
        f"🏗️ **Site Name:** {geo_info['site_name']} | **Entry:**"
        f" `{att_type.upper()}` | **Allowed Radius:** `{allowed_radius}m`"
    )
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM labor_master WHERE user_key = ?", (eng_key,)
    )
    ...
    if not labors_list:
        st.error(
            "⚠️ या साईटसाठी मास्टर लेबर एंट्री सापडली नाही. इंजिनिअरशी संपर्क"
            " साधा."
        )
        st.stop()

    labor_options = {
        f"{l['labor_name']} ({l['mobile']})": l["mobile"] for l in labors_list
    }
    selected_labor_str = st.selectbox(
        "तुमचे नाव निवडा (Select Labor Name):", list(labor_options.keys())
    )
    ...
    st.components.v1.html(
        ...
        <button onclick="getLocation()" style="width:100%; background: #00f2fe; color:#030712; padding:12px; font-weight:bold; border-radius:12px; border:none; cursor:pointer;">
            📍 Get My Current Live Location
        </button>
    """,
        height=60,
    )

    user_lat = st.query_params.get("user_lat", None)
    user_lon = st.query_params.get("user_lon", None)
    ...
    if user_lat and user_lon:
        ...
        st.write(
            f"📏 **तुमचे साईटपासून अंतर:** `{dist:.1f} मीटर` (मर्यादा:"
            f" {allowed_radius}m)"
        )

        if dist > allowed_radius:
            st.error(
                f"⚠️ तुम्ही साईटच्या {allowed_radius:.0f} मीटर"
                " परिघाबाहेर आहात! तुमची हजेरी ब्लॉक केली आहे."
            )
        else:
            st.success(
                f"✅ तुम्ही साईटच्या {allowed_radius:.0f} मीटर परिघात आहात!"
            )
            st.markdown("#### 📸 २. लाईव्ह सेल्फी काढून हजेरी लावा:")
            selfie_img = st.camera_input("Selfie Verification Camera")

            if selfie_img:
                if st.button("🚀 Submit Attendance Now", type="primary"):
                    conn = get_db_connection()
                    cursor = conn.cursor()
                    ...
                    st.balloons()
                    st.success(
                        f"🎉 {sel_labor_name}! तुमची {att_type.upper()} हजेरी"
                        " यशस्वीरित्या नोंदवली गेली आहे."
                    )
                    st.stop()
    else:
        st.warning(
            "⚠️ कृपया वरील बटणावर क्लिक करून मोबाईलचे लोकेशन चालू करा."
        )
...
if not st.session_state.welcome_completed:
    with welcome_placeholder.container():
        ...
        st.markdown(
            "<h1 style='text-align: center; color: #00f2fe; text-shadow: 0 0"
            " 20px #00f2fe;'>🌌 WELCOME TO PATIL INFRATECH</h1>",
            unsafe_allow_html=True,
        )
        st.markdown(
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM ads WHERE active = 1 AND position = 'Loading Page"
            " (Title Sponsor)'"
        )
        ...
        for i in range(5):
            status_text.markdown(
                f"<p style='text-align: center; font-size: 18px; font-weight:"
                " bold; color:"
                f" #f8fafc;'>{construction_stages[i]}</p>",
                unsafe_allow_html=True,
            )
...
if st.session_state.is_admin_logged:
    ...
    if current_tab == "rates":
        ...
        fl_calc = st.selectbox(
            ...
            ["Free", "Premium"],
            index=0
            if cur_locks.get("Civil Calculator", "Free") == "Free"
            else 1,
        )
        fl_ra = st.selectbox(
        ...
        fl_qs = st.selectbox(
            ...
            ["Free", "Premium"],
            index=0
            if cur_locks.get("Quantity Surveying", "Free") == "Free"
            else 1,
        )
        ...
        fl_geo = st.selectbox(
            "Labor Geo-Attendance Module:",
            ["Free", "Premium"],
            index=0
            if cur_locks.get("Labor Geo-Attendance", "Free") == "Free"
            else 1,
        )
        ...
        if st.button("💾 Save Feature Lock Settings", type="primary"):
            ...
            new_locks = {
                ...
                "Labor Geo-Attendance": fl_geo,
            }
            for f_name, f_lvl in new_locks.items():
                cursor.execute(
                    "REPLACE INTO feature_locks (feature_name, access_level)"
                    " VALUES (?, ?)",
                    (f_name, f_lvl),
                )
            conn.commit()
        ...
        if (
            ...
            cursor.execute(
                "SELECT code FROM premium_codes WHERE assigned_to = ? AND used"
                " = 0",
                (u_name,),
            )
            ...
            st.markdown(
                "#### 👤 MANAGE USER: <span"
                f" style='color:#ec38bc;'>{u_name.upper()}</span>",
                unsafe_allow_html=True,
            )
            st.markdown(
                f"""
            ...
            if assigned_code:
                st.info(
                    f"💡 {u_name} साठी आधीच एक कोड तयार आहे: `{assigned_code}`"
                )
            else:
                if st.button(
                    ...
                    cursor = conn.cursor()
                    cursor.execute(
                        "INSERT INTO premium_codes (code, assigned_to, used,"
                        " created_at) VALUES (?, ?, 0, ?)",
                        (new_c, u_name, now_str),
                    )
                    msg = (
                        f"तुमचा प्रिमियम कोड: {new_c} (ॲपमध्ये टाकून प्रिमियम"
                        " अनलॉक करा)"
                    )
                    cursor.execute(
                        "UPDATE users SET admin_message = ?, requested_code = 0"
                        " WHERE user_key = ?",
                        (msg, target_user),
                    )
                    conn.commit()
                    conn.close()
                    st.success(
                        f"🎉 {u_name} ला ऑटोमॅटिकली कोड पाठवला: `{new_c}`"
                    )
                    st.rerun()

            st.markdown("---")
            st.markdown(
                "##### ⏱️ प्रिमियम वेळ सेट करा / वाढवा (Custom Expiry):"
            )
            ...
            if u_prem:
                if st.button(
                    f"🔻 Revoke Premium: {u_name}",
                    key=f"win_rev_{target_user}",
                ):
                    conn = get_db_connection()
                    cursor = conn.cursor()
                    cursor.execute(
            ...
            if st.button(
                ...
                if new_msg.strip():
                    ...
                    cursor = conn.cursor()
                    cursor.execute(
                        "UPDATE users SET admin_message = ?,"
                        " unread_notification = 1 WHERE user_key = ?",
            ...
            if st.button(
                f"🗑️ Delete User: {u_name}", key=f"win_del_{target_user}"
            ):
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute(
                    "DELETE FROM users WHERE user_key = ?", (target_user,)
                )
                cursor.execute(
                    "DELETE FROM history WHERE user_key = ?", (target_user,)
                )
                conn.commit()
                conn.close()
                st.session_state.admin_view = "main"
            ...
            if u_hist:
                for idx, hist in enumerate(u_hist, 1):
                    ...
                    with st.expander(
                        f"🗓️ रिपोर्ट #{idx} | तारीख व वेळ: `{ts}`"
                    ):
                        st.markdown(
                            hist.get("report_data", "डेटा उपलब्ध नाही")
                        )
            else:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM users WHERE user_key != '9999999999' ORDER BY id"
                " ASC"
            )
            ...
            if all_users:
                ...
                for info in all_users:
                    ...
                    if u_prem:
                        col_u1.markdown(
                            ...
                            f" (User ID: <code>{u_uid}</code>)<br><small"
                        ...
                        col_u1.markdown(
                            ...
                            f" {'#10b981' if is_online else '#ef4444'};"
                            f" font-weight: bold;'>Status: {status_indicator}</small>",
                            unsafe_allow_html=True,
                        )
                    else:
                        col_u1.markdown(
                            f"<span class='free-user-badge'>🆓 FREE: {u_name.upper()}</span>"
                            f" (User ID: <code>{u_uid}</code>)<br><small"
                            f" style='color: {'#10b981' if is_online else '#ef4444'};"
                            f" font-weight: bold;'>Status: {status_indicator}</small>",
                            unsafe_allow_html=True,
                ...
                st.info(
                    "ℹ️ डेटाबेसमध्ये सध्या कोणताही सामान्य युझर नाही."
                )

    elif current_tab == "ads":
        st.markdown("### 📢 Ad & Sponsor Manager")
        st.caption(
            "💡 इथून तू दोन प्रकारचे स्पॉन्सरशिप्स (Ads) मॅनेज करू शकतोस:"
        )
        ...
        st.markdown("---")
        st.markdown(
            "##### 📋 सध्या चालू असलेल्या जाहिराती (Active Ads List):"
        )
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        if ads_list:
            for ad in ads_list:
                ...
                st.info(
                    ...
                    " *Status:*"
                    f" {'🟢 Active' if ad.get('active')==1 else '🔴 Inactive'}"
                )
                if st.button(
                    f"🗑️ Delete Ad #{ad_id}", key=f"del_ad_{ad_id}"
                ):
                    conn = get_db_connection()
                    cursor = conn.cursor()
            ...
            st.info(
                "ℹ️ सध्या कोणतीही ॲड किंवा स्पॉन्सरशिप उपलब्ध नाही."
            )

    elif current_tab == "broadcast":
        st.markdown("### 🔔 Broadcast Notification to All Users")
        st.caption(
            "💡 इथून तुम्ही एकाच वेळी सर्व रजिस्टर केलेल्या युझर्सच्या"
            " इनबॉक्समध्ये मेसेज पाठवू शकता."
        )
...
if st.session_state.app_user_name is None:
        ...
        with st.form("direct_login_form"):
            ...
            if submit_direct:
                if login_email and login_pass:
                    ...
                    cursor = conn.cursor()
                    cursor.execute(
                        "SELECT user_key FROM users WHERE (email = ? OR uid = ?"
                        " OR user_key = ?) AND pin = ?",
                    ...
                    if row:
                        ...
                        st.error(
                            "❌ चुकीचा ईमेल/Username किंवा पासवर्ड! कृपया"
                            " तपासा."
                        )
                else:
                    st.warning(
                        "⚠️ कृपया ईमेल/Username आणि पासवर्ड दोन्ही भरा."
                    )
    ...
    with otp_tab:
            ...
            if st.button("📤 Send OTP to Email", type="primary"):
                if email_input and "@" in email_input:
                    ...
                    with st.spinner("📧 ईमेलवर OTP पाठवत आहे..."):
                        ...
                        body = (
                            "नमस्कार!\n\nतुमचा पाटील इन्फ्राटेक"
                            f" लॉगिन/रेजिस्ट्रेशन OTP हा आहे: {generated_otp}\nहा"
                            " OTP कोणासोबतही शेअर करू नका.\n\n- kanhaiya"
                            " founder of patil infratech"
                        )
                        success = send_email_message(
                            email_input, subject, body
                        )

                        if success:
                            st.success(
                                "✅ तुमच्या ईमेलवर 6 अंकी OTP पाठवला आहे! खाली"
                                " टाका."
                            )
                        else:
                            st.error(
                                "❌ ईमेल पाठवताना एरर आली. (SMTP"
        ...
        if st.session_state.otp_verified and st.session_state.pending_email:
            ...
            if row:
                ...
                st.info(
                    "✨ नवीन युझर! कृपया खालील माहिती भरून युझरनेम आणि मजबूत"
                    " पासवर्ड सेट करा:"
                )
                with st.form("custom_reg_form"):
                    ...
                    custom_password = st.text_input(
                        ...
                        type="password",
                        help=(
                            "कमीत कमी ८ अक्षरे, १ अंक आणि १ विशेष चिन्ह (!@#$%)"
                            " असणे आवश्यक आहे."
                        ),
                    ).strip()
                    ...
                    if submit_custom_reg:
                        if (
                            ...
                            if custom_password != confirm_password:
                                st.error(
                                    "❌ पासवर्ड आणि कंफर्म पासवर्ड जुळत नाहीत!"
                                )
                            else:
                                is_strong, msg = is_strong_password(
                                    custom_password
                                )
                                if not is_strong:
                                    ...
                                    cursor = conn.cursor()
                                    cursor.execute(
                                        "SELECT user_key FROM users WHERE"
                                        " user_key = ? OR uid = ?",
                                    ...
                                    if cursor.fetchone():
                                        conn.close()
                                        st.error(
                                            "❌ हा Username आधीच वापरला गेला आहे,"
                                            " कृपया दुसरा टाका!"
                                        )
                                    else:
                                        welcome_msg = (
                                            f"{custom_username} मी कन्हैया आपले"
                                            " पाटील इन्फ्राटेक मध्ये आपले हार्दिक"
                                            " स्वागत आहे🥳"
                                        )
                                        now_str = get_ist_time().strftime(
                                            "%Y-%m-%d %H:%M:%S"
                                        ...
                                        subject = (
                                            "PATIL INFRATECH - Account Created"
                                            " Successfully!"
                                        )
                                        body = (
                                            f"नमस्कार {custom_username}!\n\nपाटील"
                                            " इन्फ्राटेक मध्ये तुमचे अकाउंट"
                                            " यशस्वीरित्या तयार झाले"
                                            " आहे.\n\nतुमचा लॉगिन"
                                            f" तपशील:\nUsername: {custom_username}\nPassword:"
                                            f" {custom_password}\nRegistered"
                                            " Email:"
                                            f" {st.session_state.pending_email}\n\nतुम्ही"
                                            " पुढील वेळी ईमेल/युझरनेम आणि"
                                            " पासवर्ड वापरून लॉगिन करू"
                                            " शकता.\n\n- kanhaiya founder of"
                                            " patil infratech"
                                        )
                                        send_email_message(
                                        ...
                                        st.session_state.app_user_name = (
                                            custom_username
                                        )
    ...
    with st.expander("🛡️ Admin Login Panel"):
        with st.form("admin_login_form"):
            ...
            if submit_admin:
                if (
                    admin_id == secret_admin_id
                    and admin_pass == secret_admin_pass
                ):
        ...
        with c_btn1:
            if st.button("🔓 Activate Premium", type="primary"):
                ...
                if input_code == "4528":
                    ...
                    if uses_count >= 3:
                        st.error(
                            "❌ हा मास्टर कोड तुम्ही आधीच ३ वेळा वापरला आहे!"
                            " मर्यादा संपली आहे."
                        )
                    else:
                        exp_datetime = get_ist_time() + datetime.timedelta(
                        ...
                        """,
                            (
                                ...
                                f"🎉 मास्टर कोड 4528 द्वारे तुला ८ तासांचे"
                                f" प्रिमियम मिळाले आहे! (वापर: {uses_count + 1}/3)",
                                current_user_name,
                            ),
                        )
                        conn.commit()
                        ...
                        st.success(
                            "🎉 मास्टर कोड द्वारे ८ तासांचे प्रिमियम अनलॉक"
                            " झाले!"
                        )
                        st.rerun()
                    ...
                    """,
                        (
                            exp_str,
                            "Master Code",
                            f"{current_user_name} मी कन्हैया आपले पाटील"
                            " इन्फ्राटेक मध्ये आपले हार्दिक स्वागत आहे🥳",
                            current_user_name,
                        ),
                    )
                    conn.commit()
                ...
                with st.spinner(
                    ...
                    if HAS_GENAI and api_key:
                        try:
                            ...
                            prompt = (
                                "You are a Senior Civil Engineer for Patil"
                                " Infratech. Provide a direct, professional,"
                                " final answer to the user query without"
                                f" showing calculation steps: {user_ai_query}"
                            )
                    ...
                    if not ai_response_text or "Error" in ai_response_text:
                        ai_response_text = (
                            "👷‍♂️ **Patil Infratech Expert Engineer Analysis:**"
                            f' Regarding your query *"{user_ai_query}"*, please'
                            " use our Rate Analysis or BBS Calculator modules."
                        )
...
if st.session_state.selected_module is None:
    ...
    geo_lock = locks_cfg.get("Labor Geo-Attendance", "Free")
    ...
    # --------------------------------------------------
    # SECTION 3: OTHER FEATURES (NEW LABOR GEO-ATTENDANCE ENGINE)
    # --------------------------------------------------
    st.markdown("#### 🌐 3. Other Features (Labor Geo-Attendance Engine)")
    geo_badge = "🆓 Free" if geo_lock == "Free" else "👑 Premium"
    ...
    st.markdown(
        ...
        <div style="text-align: center; background: #111827; padding: 18px 10px; border-radius: 20px; border: 1px solid rgba(236, 56, 188, 0.4); margin-bottom: 12px;">
            ...
            <p style="font-size: 10px; color: #ec38bc; margin:0;">[{geo_badge} - Live Location Verification & Dynamic Links]</p>
        </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    if st.button(
        "🌐 Open Labor Geo-Attendance System",
        key="btn_open_geo_att",
        use_container_width=True,
    ):
        if geo_lock == "Premium" and not is_user_premium:
            st.error("🔒 हे फीचर प्रिमियम युझर्ससाठी आहे!")
        else:
...
# ==========================================
    ...
    # --------------------------------------------------
    # ESTIMATOR TOOLS ICON GRID SELECTION MENU
    # --------------------------------------------------
    if st.session_state.selected_estimator_sub_module is None:
        ...
        # --------------------------------------------------
        # ESTIMATOR SUB 1: CIVIL CALCULATOR & UNIT CONVERTER
        # --------------------------------------------------
        ...
        # --------------------------------------------------
        # ESTIMATOR SUB 2: RATE ANALYSIS MODULE
        # --------------------------------------------------
        elif est_sub_mod == "Rate Analysis":
            master_rates = get_market_rates()
            st.markdown(
                "<div style='background: #111827; padding: 14px; border-radius: 16px;"
                " text-align: center; font-size: 13px; font-weight: bold; color:"
                " #f8fafc; margin-bottom: 18px; border-left: 5px solid #00f2fe; border:"
                " 1px solid rgba(0,242,254,0.2); box-shadow: 0 4px 15px"
                " rgba(0,0,0,0.5);'>"
                f"📢 आजचे मार्केट दर 🏷️ cement: ₹{master_rates.get('cement', 400.0)}/bag"
                f" | sand: ₹{master_rates.get('sand', 2500.0)}/m³ | aggregate:"
            ...
            if "Concrete Work" in main_choice:
                ...
                with l_col1:
                    mason_qty = st.number_input(
                        "मेसन संख्या (Days):", min_value=0.0, value=0.0, key="cc_msn_q"
                    )
                    mason_rate = st.number_input(
                        "मेसन दर (₹/Day):", min_value=0.0, value=600.0, key="cc_msn_r"
                    )
                with l_col2:
                    ...
                    mazdoor_rate = st.number_input(
                        "मजदूर दर (₹/Day):", min_value=0.0, value=400.0, key="cc_mzd_r"
                    )
                with l_col3:
                ...
                user_note = st.text_area(
                    "या एस्टिमेशन संदर्भात काही नोट किंवा कमेंट लिहायची असल्यास इथे लिहा:",
...
"""
                ...
                st.markdown(
                    "#### [A] साहित्याची माहिती आणि दर (क्षेत्रफळ, दर व"
                    " वॉटरप्रूफिंग)"
                )
                ...
                st.markdown("#### 💬 कमेंट पॅनल (Comment Panel)")
                user_note = st.text_area(
                    "या प्लास्टर एस्टिमेशन संदर्भात काही नोंद लिहायची असल्यास इथे लिहा:",
        ...
        # --------------------------------------------------
            ...
            with dim_col1:
                length_m = st.number_input(
                    "लांबी L (m):", min_value=0.1, value=3.0, step=0.1, key="bbs_l"
                )
            with dim_col2:
                width_m = st.number_input(
                    "रुंदी B (m):", min_value=0.1, value=0.3, step=0.05, key="bbs_b"
                )
            with dim_col3:
            ...
            st.caption(
                f"💡 **टीप:** {rcc_comp} साठी मानांकित Clear Cover **{cover} mm**"
                " आपोआप सेट केला आहे."
            )

            st.markdown(
                "#### [३] स्टील बारचे प्रकार आणि व्यास (Steel Reinforcement Details)"
            )
            ...
            if rcc_comp == "Footing":
                ...
                with c2:
                    f_dist_dia = st.selectbox(
                        "Distribution Bar DIA (mm):", dia_list, index=1, key="f_d_dia"
                    )
                ...
                with c1:
                    col_main_dia = st.selectbox(
                        "Main Vertical Bar DIA (mm):", dia_list, index=3, key="col_m_dia"
                    )
                ...
                with c2:
                    bm_bot_dia = st.selectbox(
                        "Bottom Main Bar DIA (mm):", dia_list, index=3, key="bm_b_dia"
                    )
                ...
                with c2:
                    sl_dist_dia = st.selectbox(
                        "Distribution Bar DIA (mm):", dia_list, index=0, key="sl_d_dia"
                    )
            ...
            if st.button("🧮 CALCULATE BBS REPORT", type="primary", key="bbs_calc_btn"):
                ...
                if rcc_comp == "Footing":
                    ...
                    st_cut_m = (
                        (2 * (b_net + h_net)) + (2 * hook_len) - (3 * 2 * col_st_dia)
                    ) / 1000.0
...
"""
                ...
                msg_text = "🏗️ *PATIL INFRATECH - BAR BENDING SCHEDULE (BBS)*\n"
                msg_text += f"👤 *Prepared For:* {current_user_name}\n"
                msg_text += f"📐 *Component:* {rcc_comp}\n"
                msg_text += f"📅 *Date:* {get_ist_time().strftime('%d-%m-%Y')}\n"
                msg_text += (
                    f"📐 *Size:* {length_m:.2f}m x {width_m:.2f}m x {height_m:.2f}m\n\n"
                )
                ...
                msg_text += "--------------------------------\n"
                msg_text += (
                    f"⚖️ *TOTAL STEEL WEIGHT:* {total_weight_kg:.2f} Kg"
                    f" ({total_weight_kg/1000:.3f} MT)\n"
                )
        ...
        # --------------------------------------------------
        # ESTIMATOR SUB 4: QUANTITY SURVEYING & ABSTRACT SHEET
        # --------------------------------------------------
        elif est_sub_mod == "Quantity Surveying":
            st.subheader("📈 Quantity Surveying & Abstract Sheet Master")
            st.caption(
                "💡 नोटबुकच्या मोजमाप पद्धतीनुसार खालील टेबलमध्ये Description, Nos,"
                " Length, Width, Height भरा. हिशोब तयार करा!"
            )
            ...
            with st.expander("📷 2D Plan / Blueprint / Camera Reference"):
                ...
                if "Upload" in plan_option:
                    uploaded_plan = st.file_uploader(
                        "Upload Blueprint (PNG/JPG):", type=["png", "jpg", "jpeg"]
                    )
                    if uploaded_plan:
                        st.image(
                            uploaded_plan,
            ...
            st.markdown(
                "### 🏢 Construction Stages Measurement Sheet (Excavation to Finishing)"
            )
            ...
            for idx, stg_name in enumerate(stages):
                ...
                with c_desc:
                    desc_val = st.text_input(
                        f"Description #{idx}", value=stg_name, key=f"qs_desc_{idx}"
                    )
                with c_nos:
                    nos_val = st.number_input(
                        f"Nos #{idx}", min_value=0, value=0, step=1, key=f"qs_nos_{idx}"
                    )
                with c_l:
                ...
                if nos_val > 0 and l_val > 0 and w_val > 0 and (is_area_unit or h_val > 0):
                    ...
                    st.markdown(
                        f"**📐 Single Qty: `{single_qty:.3f} {unit_label}` | Total Qty:"
                        f" `{total_qty:.3f} {unit_label}`**"
                    )
                    ...
                    if "P.C.C." in stg_name:
                        ...
                        mat_summary = (
                            f"Cement: {c_bags} Bags, Sand: {sand_m3:.2f} m³,"
                            f" Aggregate: {agg_m3:.2f} m³, Steel: {steel_kg:.1f} Kg"
                        )
                        st.info(
                            f"• **Cement:** {c_bags} Bags | **Sand:** {sand_m3:.2f} m³ |"
                        ...
                        mat_summary = (
                            f"Bricks: {bricks} Nos, Cement: {c_bags} Bags, Sand:"
                            f" {sand_m3:.2f} m³"
                        )
                        st.info(
                ...
                if is_brickwork:
                    st.markdown(
                        "##### 🚪 Brickwork Deductions (Doors / Windows in m³)"
                    )
                    ...
                    if bw_ded_vol > 0:
                        st.markdown(
                            f"**🔴 Brickwork Deduction Vol: `{bw_ded_vol:.3f} m³`**"
                        )
                ...
                if is_plaster:
                    ...
                    if st.button(
                        f"➕ Add Plaster Deduction Item #{idx}", key=f"btn_pl_ded_{idx}"
                    ):
                        st.session_state[ded_key_pl] += 1
                        st.rerun()
                    ...
                    if pl_ded_area > 0:
                        st.markdown(
                            f"**🔴 Plaster Deduction Area: `{pl_ded_area:.3f} m²`**"
                        )
            ...
            if st.button(
                ...
                if not stage_results:
                    st.warning(
                        "⚠️ कृपया कमीत कमी एका स्टेजसाठी Nos, Length, Width आणि"
                        " Height च्या व्हॅल्यू भरा!"
                    )
                else:
                    st.success(
                        "🎉 Abstract Sheet & Material Report यशस्वीरित्या तयार झाला आहे!"
                    )
                    ...
                    for r in stage_results:
                        table_rows += (
                            f"| {r['Stage']} | {r['Nos']} | {r['Dimensions']} |"
                            f" {r['TotalQty']} | {r['Material']} |\n"
                        )
                        whatsapp_text_items += (
                            f"• *{r['Stage']}*\n  - Nos: {r['Nos']} | Size:"
                            f" {r['Dimensions']}\n  - Single Qty: {r['SingleQty']:.3f}\n "
                            f" - Total Qty: {r['TotalQty']}\n  - Material:"
                            f" {r['Material']}\n\n"
                        )
...
"""
                    ...
                    msg_text = "📊 *PATIL INFRATECH - ABSTRACT SHEET*\n"
                    msg_text += f"👤 *Prepared For:* {current_user_name}\n"
                    msg_text += f"📅 *Date:* {get_ist_time().strftime('%d-%m-%Y')}\n\n"
...
# ==========================================
    ...
    if st.session_state.selected_site_sub_module is None:
        ...
        # --------------------------------------------------
        # SUB MODULE 1: DAILY ATTENDANCE & WAGES
        # --------------------------------------------------
        if sub_mod == "Attendance":
            ...
            for w_id, w_name, def_q, def_r in labor_types:
                ...
                with r_cols[3]:
                    t = q * r
                    st.markdown(
                        f"<p style='margin-top:8px; font-weight:bold;'>₹"
                        f" {t:.2f}</p>",
                        unsafe_allow_html=True,
                    )
                    total_labor_cost += t
        ...
        # --------------------------------------------------
        # SUB MODULE 2: MATERIAL INVENTORY & STOCK TRACKER
        # --------------------------------------------------
        ...
        # --------------------------------------------------
        # SUB MODULE 3: DAILY PROGRESS REPORT & PHOTO ATTACHMENT
        # --------------------------------------------------
        ...
        # --------------------------------------------------
        # SUB MODULE 4: PRE-CONCRETING CHECKLIST
        # --------------------------------------------------
            ...
            if not db_items:
                ...
                conn.commit()
                cursor.execute(
                    "SELECT id, item_text, is_checked FROM"
                    " pre_concreting_checklist WHERE user_key = ?",
                    (current_user_name,),
                )
                db_items = cursor.fetchall()
            conn.close()
            ...
            for item in db_items:
                ...
                with col_chk:
                    ...
                    if new_state != is_chk:
                        ...
                        cursor = conn.cursor()
                        cursor.execute(
                            "UPDATE pre_concreting_checklist SET is_checked = ?"
                            " WHERE id = ?",
            ...
            if st.button("🔄 चेकलिस्ट रिसेट करा (पुन्हा नवीन स्लॅबसाठी तपासणी करा)"):
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE pre_concreting_checklist SET is_checked = 0 WHERE"
                    " user_key = ?",
                    (current_user_name,),
                )
                conn.commit()
        ...
        # --------------------------------------------------
        # SUB MODULE 5: WEEKLY DASHBOARD / REPORT
        # --------------------------------------------------
        elif sub_mod == "Weekly":
            st.markdown(
            ...
            att_df = pd.read_sql_query(
                "SELECT rowid as id, date as Date, total_cost as Daily_Wage_Cost"
                f" FROM site_attendance WHERE user_key = '{current_user_name}' AND"
                f" date BETWEEN '{str_week_ago}' AND '{str_today}' ORDER BY date"
                " DESC",
                conn,
            )
            ...
            with st.expander(
                ...
                if not att_df.empty:
                    ...
                    st.markdown(
                        "**💰 एकूण ७ दिवसांचा मजुरी खर्च:** <span"
                        f" style='color:#10b981; font-size:18px;'>₹"
                        f" {total_week_wage:,.2f}</span>",
                        unsafe_allow_html=True,
                    )
                    st.dataframe(
                    ...
                    st.info(
                        "ℹ️ मागील ७ दिवसात कोणतीही हजेरी नोंदवली नाही."
                    )
            ...
            with st.expander(
                ...
                if not inv_df.empty:
                    ...
                    st.info(
                        "ℹ️ मागील ७ दिवसात कोणतेही मटेरियल IN/OUT नोंदवले"
                        " नाही."
                    )
...
# ==========================================
# 🌐 MODULE 3: LABOR GEO-ATTENDANCE SYSTEM (NEW FEATURE ENGINE)
# ==========================================
    ...
    st.write("---")
    st.subheader("📍 Labor Geo-Fence & Dynamic Attendance Control Center")

    geo_tab1, geo_tab2, geo_tab3, geo_tab4 = st.tabs([
        "१. Register Labor Master",
        "२. Site Geo-Fence Config",
        "३. Dynamic Links Generator",
        "४. Tamper-Proof Attendance Logs",
    ])

    # 1. Labor Master Registration
    with geo_tab1:
        st.markdown("#### 👥 कायमस्वरूपी कामगारांची नावे (Labor Master Entry)")
        st.caption(
            "💡 इथे एकदाच मजुराचे नाव आणि मोबाईल नंबर सेव्ह करून ठेवा."
        )

        with st.form("add_labor_form"):
            l_name = st.text_input("कामगाराचे पूर्ण नाव (Labor Name):").strip()
            l_mob = st.text_input(
                "कामगाराचा १० अंकी मोबाईल नंबर (Mobile No):", max_chars=10
            ).strip()
            sub_l = st.form_submit_button(
                "➕ Register Labor", type="primary"
            )
            ...
            if sub_l:
                if l_name and len(l_mob) == 10 and l_mob.isdigit():
                    try:
                        ...
                        cursor = conn.cursor()
                        cursor.execute(
                            "INSERT INTO labor_master (user_key, labor_name,"
                        ...
                        conn.commit()
                        conn.close()
                        st.success(f"✅ मजूर '{l_name}' नोंदवला गेला!")
                        st.rerun()
                    except sqlite3.IntegrityError:
                        st.error(
                            "❌ हा मोबाईल नंबर आधीच रजिस्टर झालेला आहे!"
                        )
                else:
                    st.warning("⚠️ कृपया नाव आणि अचूक १० अंकी मोबाईल नंबर टाका!")

        st.write("---")
        st.markdown("##### 📋 रजिस्टर मजुरांची यादी:")
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM labor_master WHERE user_key = ?",
            (current_user_name,),
        )
        ...
        if l_rows:
            for l in l_rows:
                ...
                lc1.write(f"👤 **{l['labor_name']}** (`{l['mobile']}`)")
                if lc2.button("🗑️ Delete", key=f"del_lab_{l['id']}"):
                    conn = get_db_connection()
                    cursor = conn.cursor()
                    cursor.execute(
                        "DELETE FROM labor_master WHERE id = ?", (l["id"],)
                    )
                    conn.commit()
                    conn.close()
                    st.success("🗑️ कामगार डिलीट केला!")
                    st.rerun()
        else:
            st.info("ℹ️ अजून एकही कामगार रजिस्टर केलेला नाही.")

    # 2. Geo Fence Config
    with geo_tab2:
        st.markdown("#### 📍 साईट जीपीएस लोकेशन आणि Geo-Fence सेटिंग")
        st.caption(
            "💡 साईटवर उभे राहून तुमचे लाईव्ह लोकेशन फिक्स करा व Radius सेट"
            " करा."
        )
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM site_geo_config WHERE user_key = ?",
            (current_user_name,),
        )
        ...
        site_name_val = st.text_input(
            "साईटीचे नाव (Site Name):",
            value=curr_geo_dict.get("site_name", "Main Site Project"),
        )
        radius_val = st.number_input(
            "मर्यादित अंतर परिघ (Radius in Meters):",
            min_value=10.0,
            value=float(curr_geo_dict.get("radius_meters", 100.0)),
            step=10.0,
        )
        ...
        st.components.v1.html(
            ...
            function getEngineerLocation() {
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(showEngPosition, showEngError, {enableHighAccuracy: true});
                } else {
                    alert("Geolocation is not supported by this browser.");
                }
            }
            ...
            <button onclick="getEngineerLocation()" style="width:100%; background: #00f2fe; color:#030712; padding:12px; font-weight:bold; border-radius:12px; border:none; cursor:pointer;">
                🎯 Fetch Current Engineer Site GPS Location
            </button>
        """,
            height=60,
        )

        eng_lat = st.query_params.get(
            "eng_lat", curr_geo_dict.get("latitude", 0.0)
        )
        eng_lon = st.query_params.get(
            "eng_lon", curr_geo_dict.get("longitude", 0.0)
        )

        st.write(
            f"📍 **Captured Geo Coordinates:** Latitude: `{eng_lat}` | Longitude:"
            f" `{eng_lon}`"
        )
        ...
        if st.button("💾 Save Site Geo-Fence Settings", type="primary"):
            if float(eng_lat) != 0.0 and float(eng_lon) != 0.0:
                ...
                conn.commit()
                conn.close()
                st.success("✅ साईटचे जीपीएस लोकेशन आणि जिओ-फेन्स सेव्ह झाले!")
            else:
                st.error("❌ कृपया आधी वरील बटणावर क्लिक करून लोकेशन मिळवा!")

    # 3. Dynamic Links Generator
    with geo_tab3:
        st.markdown("#### 🔗 Dynamic Link Generator (IN / OUT Links)")
        st.caption(
            "💡 एका क्लिकवर आजच्या हजेरीसाठी २ युनिक लिंक्स बनवा व व्हॉट्सॲपवर"
            " शेअर करा."
        )

        if st.button("🚀 Generate Today's Dynamic Links", type="primary"):
            ...
            cursor.execute(
                "INSERT INTO attendance_tokens (token, user_key, type, date,"
                " created_at) VALUES (?, ?, 'IN', ?, ?)",
                (in_token, current_user_name, today_date_str, now_str),
            )
            cursor.execute(
                "INSERT INTO attendance_tokens (token, user_key, type, date,"
                " created_at) VALUES (?, ?, 'OUT', ?, ?)",
                (out_token, current_user_name, today_date_str, now_str),
            )
            conn.commit()
            conn.close()

            st.success("🎉 आजच्या नवीन हजेरी लिंक्स तयार झाल्या!")

        # Show Latest Active Tokens
        conn = get_db_connection()
        cursor = conn.cursor()
        today_date_str = get_ist_time().strftime("%Y-%m-%d")
        cursor.execute(
            "SELECT * FROM attendance_tokens WHERE user_key = ? AND date = ?"
            " ORDER BY rowid DESC LIMIT 2",
            (current_user_name, today_date_str),
        )
        toks = cursor.fetchall()
        conn.close()

        if toks:
            base_app_url = "https://patil-infratech.streamlit.app"  # युझरची मूळ यूआरएल
            for t in toks:
                t_type = t["type"]
                t_code = t["token"]
                link_url = f"{base_app_url}/?att_token={t_code}"

                st.markdown(
                    f"##### 🟢 Today's {t_type} Attendance Dynamic Link:"
                )
                st.code(link_url)

                msg = (
                    f"🏗️ *PATIL INFRATECH - LIVE LABOR ATTENDANCE ({t_type.upper()})*\n"
                    f"📅 *Date:* {today_date_str}\n"
                    "👉 हजेरी लावण्यासाठी खालील लिंकवर क्लिक करा:\n"
                    f"{link_url}"
                )
                ...
                st.markdown(
                    f"""
                    <a href="https://wa.me/?text={enc_msg}" target="_blank">
                        <button style="background:#25D366; color:white; border:none; padding:8px 16px; border-radius:10px; font-weight:bold; cursor:pointer;">
                            📱 Share {t_type} Link on WhatsApp
                        </button>
                    </a>
                """,
                    unsafe_allow_html=True,
    ...
    with geo_tab4:
        st.markdown("#### 🔒 Tamper-Proof Live Labor Attendance Records")
        st.caption(
            "🛡️ ही सिस्टीम टेंपर-प्रूफ आहे. सबमिट झालेली हजेरी इंजिनिअर एडिट"
            " किंवा डिलीट करू शकत नाही. ३० दिवसांनंतर डेटा आपोआप ऑटो-क्लीन"
            " होईल."
        )
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM labor_attendance_records WHERE user_key = ? ORDER"
            " BY id DESC",
            (current_user_name,),
        )
        ...
        if logs:
            ...
            st.info("ℹ️ आज अजून एकही कामगाराने हजेरी लावलेली नाही.")
