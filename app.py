...
# सेव्ह करण्यासाठी फाईलचे नाव
DATA_FILE = "checklist_data.json"

# डीफॉल्ट पॉईंट्स
DEFAULT_ITEMS = [
    {"text": "Cover Blocks (कव्हर ब्लॉक्स) लावलेले आहेत का?", "checked": False},
    {"text": "Shuttering (शटरिंग) चा लेव्हल व सपोर्ट ओके आहे का?", "checked": False},
    {
        "text": "Electrical Conduit Pipes व जंक्शन बॉक्सेस टाकले आहेत का?",
        "checked": False,
    },
    {"text": "Curing (क्युरिंग) साठी पाण्याची योग्य सोय आहे का?", "checked": False},
    {
        "text": "सरयांचे अंतर (Reinforcement Spacing) व लॅपिंग ओके आहे का?",
        "checked": False,
    },
    {
        "text": "शटरिंग ऑइल (Shuttering Oil) लावून कचरा साफ केला आहे का?",
        "checked": False,
    },
    {
        "text": "कँक्रीट व्हायब्रेटर (Vibrator) चालू स्थितीत तयार आहे का?",
        "checked": False,
    },
]


class PreConcretingApp:

    def __init__(self, root):
        self.root = root
        self.root.title("Pre-Concreting Checklist | स्लॅब चेकलिस्ट")
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(
            self.root,
            text="🏗️ Pre-Concreting Checklist",
            font=("Arial", 16, "bold"),
            bg="#f4f7f6",
            fg="#1e293b",
        )
        ...
        subtitle_label = tk.Label(
            self.root,
            text="स्लॅब काँक्रीटिंग सुरू करण्यापूर्वीची तपासणी",
            font=("Arial", 10),
            bg="#f4f7f6",
            fg="#64748b",
        )
        ...
        self.progress_bar = ttk.Progressbar(
            status_frame, orient="horizontal", length=400, mode="determinate"
        )
        self.progress_bar.pack(fill="x", pady=5)

        self.status_badge = tk.Label(
            status_frame,
            text="🛑 काँक्रीटिंग सुरू करू नका",
            font=("Arial", 10, "bold"),
            bg="#fee2e2",
            fg="#991b1b",
            padx=10,
        ...
        add_frame = tk.Frame(self.root, bg="#f4f7f6")
        add_frame.pack(fill="x", padx=20, pady=10)

        self.add_entry = tk.Entry(add_frame, font=("Arial", 10), width=35)
        self.add_entry.pack(side="left", fill="x", expand=True, px=5)
        self.add_entry.insert(
            0, "नवीन पॉईंट जोडा..."
        ...
        checklist_container = tk.Frame(self.root, bg="#ffffff")
        checklist_container.pack(
            fill="both", expand=True, padx=20, pady=(0, 10)
        )

        self.items_frame = tk.Frame(checklist_container, bg="#ffffff")
        self.items_frame.pack(fill="both", expand=True)

        # Reset Button
        reset_btn = tk.Button(
            self.root,
            text="रिसेट करा (पुन्हा नवीन चेक करा)",
            bg="#64748b",
            fg="white",
            font=("Arial", 10, "bold"),
    ...
    def add_item(self):
        ...
        if text and text != "नवीन पॉईंट जोडा...":
    ...
    def update_progress(self):
        """प्रोग्रेस बार आणि स्टेटस अपडेट करणे"""
        total = len(self.items)
        if total == 0:
            completed = 0
            percent = 0
        else:
            completed = sum(1 for var in self.check_vars if var.get())
            percent = int((completed / total) * 100)

        self.progress_bar["value"] = percent
        self.progress_label.config(
            text=f"पूर्णता: {percent}% ({completed}/{total})"
        )

        if percent == 100 and total > 0:
            self.status_badge.config(
                text="✅ काँक्रीटिंग सुरू करण्यास पूर्ण परवानगी आहे!",
                bg="#dcfce7",
                fg="#166534",
            )
        else:
            self.status_badge.config(
                text="🛑 काँक्रीटिंग सुरू करू नका (अपूर्ण तपासणी)",
                bg="#fee2e2",
                fg="#991b1b",
            )
    ...
    def reset_checklist(self):
        """सर्व टिक मार्क्स काढणे"""
        if messagebox.askyesno(
            "रिसेट करा", "तुम्हाला सर्व टिक-मार्क्स काढून टाकायचे आहेत का?"
        ):
...
# मुख्य ॲप्लिकेशन चालवणे
if __name__ == "__main__":
    root = tk.Tk()
    app = PreConcretingApp(root)
    root.mainloop()
