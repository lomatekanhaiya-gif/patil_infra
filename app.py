...
# Step 1: Component & Unit Selection
    col_comp, col_unit = st.columns(2)
    with col_comp:
        rcc_component = st.selectbox("1️⃣ RCC Component निवडा:", ["Footing", "Column", "Beam", "Slab"], key="bbs_comp")
    with col_unit:
        unit = st.selectbox("2️⃣ युनिट निवडा (Unit):", ["Meter (m)", "Feet (ft)"], key="bbs_unit")

    # Standard Cover Mapping (mm)
    standard_covers = {
        "Footing": 50,
        "Column": 40,
        "Beam": 25,
    ...
    default_cov = standard_covers.get(rcc_component, 25)

    st.markdown("---")
    st.markdown(f"#### 📐 {rcc_component} आकार आणि कव्हर माहिती")

    # Dynamic Dimensions Inputs
    col_l, col_b, col_h = st.columns(3)
    unit_label = "m" if "Meter" in unit else "ft"

    with col_l:
        dim_l = st.number_input(f"लांबी L ({unit_label}):", min_value=0.0, value=3.0, step=0.1, key="bbs_l")
    with col_b:
        dim_b = st.number_input(f"रुंदी B ({unit_label}):", min_value=0.0, value=0.3, step=0.05, key="bbs_b")
    with col_h:
        dim_h = st.number_input(f"उंची/जाडी H ({unit_label}):", min_value=0.0, value=0.45, step=0.05, key="bbs_h")

    col_cov, col_no = st.columns(2)
    with col_cov:
        clear_cover = st.number_input("Clear Cover (mm):", min_value=0, value=default_cov, key="bbs_cover")
    with col_no:
        total_nos = st.number_input(f"एकूण {rcc_component} संख्या (Quantity):", min_value=1, value=1, step=1, key="bbs_nos")

    st.markdown("---")
    st.markdown("#### 🔩 Bar Details (सळईचे तपशील)")

    col_dia, col_spacing = st.columns(2)
    with col_dia:
        main_dia = st.selectbox("मुख्य स्टील व्यास (Main Bar Dia in mm):", [8, 10, 12, 16, 20, 25, 32], index=2, key="bbs_main_dia")
    with col_spacing:
        main_nos_or_spacing = st.number_input("मुख्य स्टील नग किंवा अंतर Spacing (mm):", min_value=1, value=4 if rcc_component in ["Column", "Beam"] else 150, key="bbs_main_ns")

    # Secondary / Stirrup Bar Details based on Component
    secondary_bar = False
    if rcc_component in ["Column", "Beam"]:
        secondary_bar = True
        st.markdown("##### 🌀 Ring / Stirrup Details")
        st_c1, st_c2 = st.columns(2)
        with st_c1:
            stirrup_dia = st.selectbox("रिंगचा व्यास (Stirrup Dia in mm):", [6, 8, 10, 12], index=1, key="bbs_st_dia")
        with st_c2:
            stirrup_spacing = st.number_input("रिंगमधील अंतर Spacing (mm):", min_value=50, value=150, step=10, key="bbs_st_sp")
    elif rcc_component == "Slab":
        secondary_bar = True
        st.markdown("##### 🌐 Distribution Bar Details")
        dist_c1, dist_c2 = st.columns(2)
        with dist_c1:
            dist_dia = st.selectbox("डिस्ट्रिब्युशन व्यास (Distribution Dia in mm):", [8, 10, 12], index=0, key="bbs_dist_dia")
        with dist_c2:
            dist_spacing = st.number_input("डिस्ट्रिब्युशन अंतर Spacing (mm):", min_value=50, value=150, step=10, key="bbs_dist_sp")

    st.markdown("#### 💬 कमेंट पॅनल (Comment Panel)")
    user_note = st.text_area("या BBS रिपोर्ट संदर्भात टीप/कमेंट लिहा:", placeholder="उदा. C1 Column BBS - Ground Floor", key="bbs_note")

    if st.button("🏗️ GENERATE BBS REPORT", type="primary", key="btn_gen_bbs"):
        # Convert Feet to Meters internally for standardized IS code formula
        unit_factor = 0.3048 if "Feet" in unit else 1.0
        L_m = dim_l * unit_factor
        B_m = dim_b * unit_factor
        H_m = dim_h * unit_factor
        cover_m = clear_cover / 1000.0

        rows = []

        # ==========================================
        # LOGIC CALCULATION ACCORDING TO IS CODE
        # ==========================================
        if rcc_component == "Footing":
            # Main Bars (Mesh L & B directions)
            # Length wise bars
            cut_l = (L_m - 2 * cover_m) + 2 * (12 * (main_dia / 1000.0))  # Length + 2 hooks/bends
            nos_l = math.ceil((B_m - 2 * cover_m) / (main_nos_or_spacing / 1000.0)) + 1
            tot_len_l = cut_l * nos_l * total_nos
            wt_l = (main_dia ** 2 / 162.2) * tot_len_l

            rows.append({
                "SRNO": 1,
                "DESCRIPTION": "Footing Mesh (Longitudinal)",
                "NOS": nos_l * total_nos,
                "DIA": f"{main_dia} mm",
                "LENGTH": f"{cut_l:.2f} m",
                "TOTAL LENGTH": f"{tot_len_l:.2f} m",
                "WEIGHT": f"{(main_dia ** 2 / 162.2):.3f} kg/m",
                "TOTAL WEIGHT": f"{wt_l:.2f} Kg"
            })

            # Width wise bars
            cut_b = (B_m - 2 * cover_m) + 2 * (12 * (main_dia / 1000.0))
            nos_b = math.ceil((L_m - 2 * cover_m) / (main_nos_or_spacing / 1000.0)) + 1
            tot_len_b = cut_b * nos_b * total_nos
            wt_b = (main_dia ** 2 / 162.2) * tot_len_b

            rows.append({
                "SRNO": 2,
                "DESCRIPTION": "Footing Mesh (Transverse)",
                "NOS": nos_b * total_nos,
                "DIA": f"{main_dia} mm",
                "LENGTH": f"{cut_b:.2f} m",
                "TOTAL LENGTH": f"{tot_len_b:.2f} m",
                "WEIGHT": f"{(main_dia ** 2 / 162.2):.3f} kg/m",
                "TOTAL WEIGHT": f"{wt_b:.2f} Kg"
            })

        elif rcc_component == "Column":
            # Main Vertical Bars
            # Cut Length = Height + L-bend (50d) - Cover
            cut_main = H_m + (50 * (main_dia / 1000.0)) - cover_m
            tot_len_main = cut_main * main_nos_or_spacing * total_nos
            wt_main = (main_dia ** 2 / 162.2) * tot_len_main

            rows.append({
                "SRNO": 1,
                "DESCRIPTION": "Column Main Vertical Bars",
                "NOS": int(main_nos_or_spacing * total_nos),
                "DIA": f"{main_dia} mm",
                "LENGTH": f"{cut_main:.2f} m",
                "TOTAL LENGTH": f"{tot_len_main:.2f} m",
                "WEIGHT": f"{(main_dia ** 2 / 162.2):.3f} kg/m",
                "TOTAL WEIGHT": f"{wt_main:.2f} Kg"
            })

            # Column Stirrups (Rings)
            a = L_m - 2 * cover_m
            b = B_m - 2 * cover_m
            # Cut Length = 2*(a+b) + 2*10d (hook) - 3*2d (90 deg bend) - 2*3d (135 deg bend)
            cut_st = 2 * (a + b) + (2 * 10 * (stirrup_dia / 1000.0)) - (6 * (stirrup_dia / 1000.0))
            nos_st = math.ceil((H_m - 2 * cover_m) / (stirrup_spacing / 1000.0)) + 1
            tot_len_st = cut_st * nos_st * total_nos
            wt_st = (stirrup_dia ** 2 / 162.2) * tot_len_st

            rows.append({
                "SRNO": 2,
                "DESCRIPTION": "Column Rectangular Stirrups",
                "NOS": nos_st * total_nos,
                "DIA": f"{stirrup_dia} mm",
                "LENGTH": f"{cut_st:.2f} m",
                "TOTAL LENGTH": f"{tot_len_st:.2f} m",
                "WEIGHT": f"{(stirrup_dia ** 2 / 162.2):.3f} kg/m",
                "TOTAL WEIGHT": f"{wt_st:.2f} Kg"
            })

        elif rcc_component == "Beam":
            # Main Longitudinal Bars
            cut_main = L_m - 2 * cover_m + 2 * (50 * (main_dia / 1000.0))  # L + 2 * Ld (50d)
            tot_len_main = cut_main * main_nos_or_spacing * total_nos
            wt_main = (main_dia ** 2 / 162.2) * tot_len_main

            rows.append({
                "SRNO": 1,
                "DESCRIPTION": "Beam Main Longitudinal Bars",
                "NOS": int(main_nos_or_spacing * total_nos),
                "DIA": f"{main_dia} mm",
                "LENGTH": f"{cut_main:.2f} m",
                "TOTAL LENGTH": f"{tot_len_main:.2f} m",
            ...
            b = H_m - 2 * cover_m
            cut_st = 2 * (a + b) + (2 * 10 * (stirrup_dia / 1000.0)) - (6 * (stirrup_dia / 1000.0))
            nos_st = math.ceil((L_m - 2 * cover_m) / (stirrup_spacing / 1000.0)) + 1
            tot_len_st = cut_st * nos_st * total_nos
            wt_st = (stirrup_dia ** 2 / 162.2) * tot_len_st

            rows.append({
                "SRNO": 2,
                "DESCRIPTION": "Beam Stirrups (Rings)",
                "NOS": nos_st * total_nos,
                "DIA": f"{stirrup_dia} mm",
                "LENGTH": f"{cut_st:.2f} m",
                "TOTAL LENGTH": f"{tot_len_st:.2f} m",
            ...
            # Main Crank / Bent-up Bars
            d_eff = H_m - 2 * cover_m
            cut_main = (L_m - 2 * cover_m) + (0.42 * d_eff) + 2 * (9 * (main_dia / 1000.0))
            nos_main = math.ceil((B_m - 2 * cover_m) / (main_nos_or_spacing / 1000.0)) + 1
            tot_len_main = cut_main * nos_main * total_nos
            wt_main = (main_dia ** 2 / 162.2) * tot_len_main

            rows.append({
                "SRNO": 1,
                "DESCRIPTION": "Slab Main Bent-up Bars",
                "NOS": nos_main * total_nos,
                "DIA": f"{main_dia} mm",
                "LENGTH": f"{cut_main:.2f} m",
                "TOTAL LENGTH": f"{tot_len_main:.2f} m",
                "WEIGHT": f"{(main_dia ** 2 / 162.2):.3f} kg/m",
            ...
            # Distribution Bars
            cut_dist = (B_m - 2 * cover_m) + 2 * (9 * (dist_dia / 1000.0))
            nos_dist = math.ceil((L_m - 2 * cover_m) / (dist_spacing / 1000.0)) + 1
            tot_len_dist = cut_dist * nos_dist * total_nos
            wt_dist = (dist_dia ** 2 / 162.2) * tot_len_dist

            rows.append({
                "SRNO": 2,
                "DESCRIPTION": "Slab Distribution Bars",
                "NOS": nos_dist * total_nos,
                "DIA": f"{dist_dia} mm",
                "LENGTH": f"{cut_dist:.2f} m",
                "TOTAL LENGTH": f"{tot_len_dist:.2f} m",
                "WEIGHT": f"{(dist_dia ** 2 / 162.2):.3f} kg/m",
                "TOTAL WEIGHT": f"{wt_dist:.2f} Kg"
            })

        # Display Report
        st.success("🎉 BBS रिपोर्ट यशस्वीरित्या जनरेट झाला आहे!")
        st.markdown(f"### 🏗️ BAR BENDING SCHEDULE (BBS) REPORT")
        st.info(f"👤 **Prepared For:** {current_user_name} | **Component:** {rcc_component} | **Clear Cover:** {clear_cover} mm | **Quantity:** {total_nos} Nos")

        # Convert to Markdown Table for standard output
        df_bbs = pd.DataFrame(rows)
        report_table = df_bbs.to_markdown(index=False)
        st.markdown(report_table)

        # Calculate total weight
        grand_total_weight = sum([float(r["TOTAL WEIGHT"].replace(" Kg", "")) for r in rows])
        st.markdown(f"### ⚖️ **एकूण स्टीलचे वजन (GRAND TOTAL WEIGHT): `{grand_total_weight:.2f} Kg` ({grand_total_weight/1000:.3f} MT)**")

        # Save into user history
        user_db = load_db()
        if current_user_name in user_db:
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            full_report_text = f"**BBS REPORT ({rcc_component})**\n\n" + report_table + f"\n\n**Total Steel Weight: {grand_total_weight:.2f} Kg**"
            new_report = {
                "timestamp": timestamp,
                "user_note": user_note.strip() if user_note.strip() else "BBS Report",
                "report_data": full_report_text
            }
            user_db[current_user_name]["history"].append(new_report)
            save_db(user_db)
